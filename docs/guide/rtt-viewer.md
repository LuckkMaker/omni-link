...
RTT 提供两种将接收数据保存到文件的方式：

- **接收数据到文件** — 点击接收区「接收数据到文件」按钮，选择 `.dat` 文件后，每帧接收到的数据会持续追加写入该文件，可实现长时间连续录制。录制过程中按钮高亮，再次点击停止并保存。
- **保存数据** — 点击「保存数据」按钮，将当前 tab 已接收的缓冲数据一次性导出到文件，格式支持：
  - **Raw** — 原始字节流
  - **HEX** — 十六进制文本
  - **Text** — ASCII 文本（按通道分隔）
