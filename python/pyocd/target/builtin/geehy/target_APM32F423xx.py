...
#
# APM32F423 generic target: 1MB Flash, 320KB SRAM + 64KB CCM.
# Replaces APM32F423ZG/VG/RG/CG (same flash capacity).
#
...
class APM32F423xG(CoreSightTarget):
    """APM32F423: 1MB Flash, 320KB SRAM + 64KB CCM."""
    ...
    MEMORY_MAP = MemoryMap(
        ...
        FlashRegion(start=0x08020000, length=0xe0000, sector_size=0x20000,
                    page_size=0x400,
                    erase_all_weight=CHIP_ERASE_WEIGHT, algo=FLASH_ALGO_1MB),
        RamRegion(start=0x20000000, length=0x50000),
        RamRegion(start=0x10000000, length=0x10000),
    )

    def __init__(self, session):
        super().__init__(session, self.MEMORY_MAP)
        self._svd_location = SVDFile.from_builtin("APM32F423.svd")
