...
class STM32F107RB(CoreSightTarget):
    ...
    def __init__(self, session):
        super(STM32F107RB, self).__init__(session, self.MEMORY_MAP)
        self._svd_location = SVDFile.from_builtin("STM32F107xx.svd")

    def post_connect_hook(self):
        self.write_memory(DBGMCU_CR, DBGMCU_VAL)


class STM32F107RC(CoreSightTarget):
    ...
    def __init__(self, session):
        super(STM32F107RC, self).__init__(session, self.MEMORY_MAP)
        self._svd_location = SVDFile.from_builtin("STM32F107xx.svd")
...
class STM32F107VC(CoreSightTarget):
    ...
    def __init__(self, session):
        super(STM32F107VC, self).__init__(session, self.MEMORY_MAP)
        self._svd_location = SVDFile.from_builtin("STM32F107xx.svd")
