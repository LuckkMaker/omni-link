 ...
 * - 定义 omni-dark 主题，对齐 Tailwind 深色配色
 */
...
// ── omni-dark 主题（对齐应用 Tailwind 主题色变量）──────────────────
...
/** 读取 CSS 变量并应用为 Monaco 主题；返回主题名 */
export function applyOmniTheme(): string {
...
// 模块加载时立即应用主题（兜底，保证 theme="omni-dark" 一定可用）
try {
  applyOmniTheme()
} catch {
  /* 非浏览器环境跳过 */
